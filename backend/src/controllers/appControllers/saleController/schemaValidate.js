...
const schema = Joi.object({
  client: Joi.alternatives().try(Joi.string(), Joi.object()).required(), // Customer can be a string (ID) or an object
  number: Joi.number().required(), // Sale number (required)
  ...
  date: Joi.date().required(), // Sale date (required)
  expectedDeliveryDate: Joi.date(), // Expected delivery date (optional)
  salesOrderNumber: Joi.string().allow(''), // Sales order number (optional)
  items: Joi.array() // Items array (required)
    ...
    .required(),
  taxRate: Joi.alternatives().try(Joi.number(), Joi.string()), // Tax rate (required)
